...
対象は `## Test plan` セクションに限らず、`## 追加テスト観点` セクション配下の
`- [ ] [AI確認] <条件>` 項目も同様に検証対象です（Issue #2929）。
...
- 呼び出し元は本 subagent の JSON 出力を受け取り、`verified=true` の項目のチェックボックスを `- [x]` に更新して `gh pr edit <N> --body-file <更新後本文ファイル>` で反映する
