...
**引数なし**で実行した場合、Issue 選定の前に以下を実行して並行 PR 数を確認する。

```bash
uv run --project projects/py/tidd_tools python -m tidd_tools check-pr-conflicts --count-only
...
**引数あり（単一番号・バッチモード）の場合は STEP 0 をスキップする。**
...
2. 状態を JSON ファイルに永続化する（STEP 1.5 より先に実行）:
   ```bash
   uv run --project projects/py/tidd_tools python -m tidd_tools issue-next-state migrate
   uv run --project projects/py/tidd_tools python -m tidd_tools issue-next-state init 42 43 44
...
4. マージ完了後は未処理キューの先頭を消費して次の着手対象とする:
   ```bash
   next=$(uv run --project projects/py/tidd_tools python -m tidd_tools issue-next-state consume)
   ```
...
```bash
uv run --project projects/py/tidd_tools python -m tidd_tools issue-next-state current   # current_issue を表示
uv run --project projects/py/tidd_tools python -m tidd_tools issue-next-state queue     # 残キュー（空白区切り）を表示
...
STEP 1 で着手対象 Issue が確定した直後に、**ラベル有無にかかわらず毎回**品質チェックを実行する。
...
着手対象 Issue に `duplicate-suspect` ラベルが付いている場合のみ実行する。
...
STEP 2 前に `tidd check-pr-conflicts --issue <N>` を実行する（exit 4 = 競合 PR あり）:
...
worktree に `cd` した直後に `.venv` を初期化する（#2035）。省略すると Python 3.14 + dev extra 抜きの `.venv` が作られ、後続の ai-review が `Failed to spawn: ruff` で失敗する。
...
uv run --project projects/py/tidd_tools python -m tidd_tools check-pr-conflicts "${CHANGED[@]}"
# 0 → 競合なし。STEP 4 へ
...
```bash
uv run --project projects/py/tidd_tools python -m tidd_tools transfer-issue-items <PR番号>
...
uv run --project projects/py/tidd_tools python -m tidd_tools ai-review <PR番号> <試行回数>
```
...
3. 試行回数をインクリメントして再実行する:
   ```bash
   uv run --project projects/py/tidd_tools python -m tidd_tools ai-review <PR番号> 2  # 2回目
   uv run --project projects/py/tidd_tools python -m tidd_tools ai-review <PR番号> 3  # 3回目
   ```
...
**引数なし（自動ループ）:** 「PR #N は人間マージが必要です（AIレビュー解決不能）」と記録し、STEP 1 に戻って次の Issue を選定する。STEP 0 の並行 PR 数上限チェックが再度行われる。
...
# 失敗（引数なし）→ 「PR #N は CI 失敗のため人間マージが必要です」と記録しSTEP 1へ
# 失敗（単一番号）→ 「PR #N は CI 失敗のため人間マージが必要です」と報告して終了
# 失敗（バッチ）→ 「PR #N は CI 失敗。残りのキュー [...] は処理されませんでした」と報告して終了
```
...
- 単一番号指定 → 「Issue #N 完了」をメインセッションに報告して終了する
