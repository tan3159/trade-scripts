...
`gh pr view <PR番号> --json state` の state が `MERGED` でない場合（未マージ・conflict・API エラー等）:
...
3. Issue に以下の文言をコメント投稿する（`.claude/rules/escalation-format.md` 準拠）:
   ...
   >
   > A. PR の状態を再確認してから worktree・branch を削除する — `gh pr view --json state` で state を確認し MERGED なら `uv run --project projects/py/tidd_tools tidd cleanup-merged-branch <branch>` を実行（推奨）
...
1. **park 理由が対象 Issue のスコープ外である別バグ（ブロッカー）だと判断できる場合:**
   1. そのバグ用の Issue を自動起票する（`gh issue create`・タイトル `fix: <バグ概要> を修正する`・ラベル `type: fix`・`priority: critical`・`source: rework`・`issue-creation.md` 準拠の `## 背景`・`## やること`・`## 振る舞い`）
