...
`gh pr list --state open --json number,body` で全 open PR を取得し、
...
- `gh pr list --state open --json number,body` で PR 一覧を取得
- `body` に `closes #<N>` を含む PR を filter して `number` を取得（`_pr_num`）
- `gh pr view <_pr_num> --json headRefName` で `headRefName` を取得（`_head_ref`）
