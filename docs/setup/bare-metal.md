...
**注:** `mise` を使わずに uv 単体で Python バージョンを解決する。`uv tool install` 実行時に必要な Python が自動で
downloads される。

### 2. tidd のインストール

このドキュメントの対象は **copier で生成した consumer 自身のリポジトリ**であり、`projects/py/tidd_tools` は存在しない。`tidd` コマンドは ai-dev-handbook 本体を `uv tool install` で直接インストールして PATH から使う（[`copier-workflow-adoption.md` §3](https://github.com/being-gaia-plan/ai-dev-handbook/blob/main/docs/setup/copier-workflow-adoption.md) と同じ方式）。

```bash
uv tool install "tidd-tools @ git+https://github.com/being-gaia-plan/ai-dev-handbook@main#subdirectory=projects/py/tidd_tools"
```
...
- 実装: [`projects/py/tidd_tools/src/tidd_tools/shared/secrets.py`](https://github.com/being-gaia-plan/ai-dev-handbook/blob/main/projects/py/tidd_tools/src/tidd_tools/shared/secrets.py)（`get_secret()`・`_has_env_value()`。ai-dev-handbook 本体のみに存在し consumer には配布されない）
...
uv tool install "tidd-tools @ git+https://github.com/being-gaia-plan/ai-dev-handbook@main#subdirectory=projects/py/tidd_tools" --python 3.11
```
...
Issue #1315 の Bitwarden fallback が該当サブコマンドに未対応の可能性。`projects/py/tidd_tools` は consumer リポジトリには存在しないため、[GitHub 上のソース検索](https://github.com/search?q=repo%3Abeing-gaia-plan%2Fai-dev-handbook+%22bw+get+password%22&type=code)で確認する。
