...
### CLI 非管理（hooks-config.json に手動追記で on・6 個）

| hook | 有効化理由 |
|------|-----------|
| `require-issue-id-in-pr-title` | PR タイトル規約 `<type>(<scope>): #N` の機械強制 |
| `require-merge-ci-status` | CI ❌ 状態でのマージ防止（CI 導入 Issue #8 の前提） |
| `require-mypy` | Python 型チェックゲート（projects/py 雛形 Issue #6 の前提） |
| `require-ruff-format` | Python フォーマットゲート（同上） |
| `validate-skill` | `.claude/skills/` 編集時の構造検証 |
| `notify-copier-staleness` | 上流テンプレート追従の staleness 通知（copier update 運用の前提） |
