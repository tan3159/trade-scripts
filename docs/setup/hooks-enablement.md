...
| `require-issue-id-in-pr-title` | PR タイトル規約 `<type>(<scope>): #N` の機械強制 | 発動する（v2026.07.19 で settings.json 登録・#24） |
| `require-mypy` | Python 型チェックゲート（projects/py 雛形 Issue #6 の前提） | 発動するが**実質 skip**（下記注意参照） |
| `require-ruff-format` | Python フォーマットゲート（同上） | 発動するが**実質 skip**（同） |

**注意（既知の上流問題）:** `require-mypy` / `require-ruff-format` は settings.json 登録済み
（上流 #2229 の修正が v2026.07.19 で反映・#24）だが、検査対象が `projects/py/tidd_tools` に
ハードコードされており本リポジトリには存在しないため、常に WARN + skip（exit 0）となり
ゲートとして機能しない（上流 ai-dev-handbook#2278 で修正依頼済み）。
上流修正が次期タグの copier update で反映され次第そのまま有効になる。
...
- `require-issue-id-in-pr-title`: Issue ID なしタイトルの `gh pr create` → **exit 2 でブロック**
  （v2026.07.19 反映後の再確認（2026-07-19・#24）でも同様にブロックを確認。settings.json 登録済みのためセッション内でも発動する）
