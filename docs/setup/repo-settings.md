...
- **required_status_checks** — GitHub Actions `python` job（ci.yml・pytest/ruff/mypy）の成功をマージ条件にする（2026-07-20 適用・Issue #21）。ci.yml は path filter なしで全 PR に走るため docs-only PR も阻害しない。strict policy（up-to-date 必須）は off（squash マージ運用では過剰なため）
...
`integration_id: 15368` は GitHub Actions の App ID（Actions 以外が投稿した同名 status を認めないため固定する）。
既存 ruleset への追加は `gh api -X PUT "repos/$REPO/rulesets/<id>"` に rules 全量を渡して行う。
